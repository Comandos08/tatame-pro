...
function extractUsedKeys(srcDir) {
  ...
  // *Key="value" or *Key='value' prop patterns (string literals only).
  // Excluded: featureKey (feature-access flags, not i18n), JSX expressions {var.prop}.
  const keyProp = /(?:titleKey|descriptionKey|labelKey|i18nKey|translationKey|messageKey|placeholderKey|actionKey|hintKey)\s*=\s*["']([^"']+)["']/g;
  ...
  for (const file of files) {
    ...
    while ((m = keyProp.exec(content)) !== null)  used.add(m[1]);
  }
