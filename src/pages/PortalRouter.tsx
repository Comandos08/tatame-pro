 ...
 * 
 * REGRAS IMUTÁVEIS:
 * 1. Usuário não autenticado → /login (único lugar que redireciona para login)
...
export default function PortalRouter() {
  ...
  const resolveDestination = async () => {
    try {
      ...
      // Se tem papel administrativo em algum tenant
      ...
      // Se é atleta vinculado a um tenant
      ...
      // 5️⃣ Buscar membership por applicant_profile_id (candidato ainda não aprovado)
      ...
      // NÃO redirecionar para / ou /login — exibir estado neutro
  ...
  const redirectToTenantAdmin = async (tenantId: string, tenantSlug: string) => {
    try {
      ...
      // Resolver destino via função pura
