...
export default function AthleteLogin() {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    if (!tenantSlug) {
      setError(t("auth.magicLink.invalidTenant"));
      setIsLoading(false);
      return;
    }

    // Sanitizar redirect: respeita ?redirect apenas se for do tenant
    const rawRedirect = searchParams.get("redirect");
    const safeRedirect =
      rawRedirect && rawRedirect.startsWith(`/${tenantSlug}`) ? rawRedirect : `/${tenantSlug}/portal`;

    const redirectUrl = `${window.location.origin}/auth/callback?next=${encodeURIComponent(safeRedirect)}`;
          ...
          <CardHeader className="text-center">
            ...
            {!emailSent ? (
              <>
                ...
                <CardDescription>{t("auth.magicLink.description")}</CardDescription>
              </>
            ) : (
              <>
                <motion.div
                ...
                <CardTitle className="text-2xl font-display">{t("auth.magicLink.success")}</CardTitle>
                <CardDescription>{t("auth.magicLink.successDesc")}</CardDescription>
              </>
            )}
          </CardHeader>
            ...
            {!emailSent ? (
              ...
              <div className="space-y-4">
                <div className="bg-muted/50 rounded-lg p-4 text-center">
                  <p className="text-sm text-muted-foreground">{t("auth.magicLink.checkSpam")}</p>
                </div>
