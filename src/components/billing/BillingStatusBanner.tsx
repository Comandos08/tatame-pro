...
export function BillingStatusBanner() {
  ...
  const canSeeBilling =
    tenant?.id && currentUser && (hasRole("ADMIN_TENANT", tenant.id) || hasRole("SUPERADMIN_GLOBAL"));
  ...
  // 🔥 ALTERAÇÃO AQUI (Prompt 4 de 5)
  if (!billing) {
    return (
      <Alert variant="default" className="mb-6">
        <CreditCard className="h-4 w-4" />
        <AlertTitle>{t("billing.setup.bannerTitle")}</AlertTitle>
        <AlertDescription>{t("billing.setup.bannerDescription")}</AlertDescription>
      </Alert>
    );
  }
  ...
  const canManagePayment =
    billing.stripe_customer_id && ["TRIALING", "PAST_DUE", "INCOMPLETE", "UNPAID"].includes(billing.status);
    ...
    >
      ...
      <AlertDescription>
        ...
        {periodEnd && billing.status !== "CANCELED" && (
          <span className="block mt-1 text-sm opacity-80">{t("billing.validUntil").replace("{date}", periodEnd)}</span>
        )}
