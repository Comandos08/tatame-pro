...
export function PortalAccessGate({ athlete, membership, isLoading, error, children }: PortalAccessGateProps) {
  const { t } = useI18n();
  ...
  const getGateState = (): GateState => {
    ...
    const isEffectivelyExpired =
      status === "EXPIRED" || (membership.end_date && new Date(membership.end_date) < new Date());
  ...
  const stateConfig: Record<
    Exclude<GateState, "loading" | "allowed">,
    {
      ...
      >
        <Card className="max-w-md mx-auto">
          <CardContent className="pt-8 pb-8 text-center">
            <div className={`h-16 w-16 rounded-full ${config.iconBg} flex items-center justify-center mx-auto mb-4`}>
